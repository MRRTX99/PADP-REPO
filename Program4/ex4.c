...
int main(int argc, char *argv[]){
    ...
    #pragma omp parallel for schedule(guided,100) private(x,y,red,blue,green,temp,color) /*schedule(static,50)*/ /* schedule(dynamic,50) */ 
    							/*schedule(guided, 50) */
    for(x = 0; x<w; x++)
        for(y = 0; y<h; y++)
            {
            //tid =3;      //uncomment when usung private()      
            tid = omp_get_thread_num(); //get thread number    //uncomment for general output
